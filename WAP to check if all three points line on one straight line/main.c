...
int main()
{
    ...
    float slope1=(float)(y2-y1)/(x2-x1);//The (float) you see in lines 9 and 10 is called a typecast or type conversion. It's used to explicitly convert the result of the division operation from an integer to a float. Here's why it's necessary:
...
//Typecasting to Float: To ensure that you get a floating-point result with the fractional part, you explicitly cast one or both operands to float. In your case, (float)(y2 - y1) / (x2 - x1) and (float)(y3 - y2) / (x3 - x2) cast the results of the divisions to floating-point numbers.
